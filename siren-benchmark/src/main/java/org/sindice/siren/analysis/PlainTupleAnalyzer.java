...
/**
 * TupleAnalyzer that only defines the {@link TupleTokenizer} wihtout any
 * filters.
 */
public class PlainTupleAnalyzer extends Analyzer {

  @Override
  public final TokenStream tokenStream(final String fieldName, final Reader reader) {
    final TupleTokenizer stream = new TupleTokenizer(reader, Integer.MAX_VALUE);
    final TokenStream result = new TokenTypeFilter(stream, new int[] {TupleTokenizer.BNODE,
                                                                TupleTokenizer.DOT});
    return result;
  }
