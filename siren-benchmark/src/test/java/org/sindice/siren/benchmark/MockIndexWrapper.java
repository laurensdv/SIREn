/**
 * Copyright (c) 2009-2012 National University of Ireland, Galway. All Rights Reserved.
 *
 * Project and contact information: http://www.siren.sindice.com/
 *
 * This file is part of the SIREn project.
 *
 * SIREn is a free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as
 * published by the Free Software Foundation, either version 3 of
 * the License, or (at your option) any later version.
 *
 * SIREn is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public
 * License along with SIREn. If not, see <http://www.gnu.org/licenses/>.
 */
/**
 * @project siren-benchmark
 * @author Renaud Delbru [ 6 Jul 2012 ]
 * @link http://renaud.delbru.fr/
 */
package org.sindice.siren.benchmark;

import java.io.IOException;
import java.util.List;

import org.sindice.siren.benchmark.generator.document.BenchmarkDocument;
import org.sindice.siren.benchmark.generator.lexicon.TermFreqIterator;
import org.sindice.siren.benchmark.query.provider.Query;
import org.sindice.siren.benchmark.query.task.MultiQueryTask;
import org.sindice.siren.benchmark.query.task.QueryTask;
import org.sindice.siren.benchmark.query.task.SingleQueryTask;
import org.sindice.siren.benchmark.wrapper.IndexWrapper;

public class MockIndexWrapper implements IndexWrapper {

  public MockIndexWrapper() {}

  @Override
  public void addDocument(final BenchmarkDocument document)
  throws IOException {}

  @Override
  public void close() throws IOException {}

  @Override
  public void commit() throws IOException {}

  @Override
  public void forceMerge() throws IOException {}

  @Override
  public void flushCache() {}

  @Override
  public QueryTask issueQuery(final Query query) {
    return new SingleQueryTask(query) {

      @Override
      public Measurement call() {
        final Measurement measurement = new Measurement(100);
        measurement.setCPUTime(1000000000);
        return measurement;
      }

      @Override
      public void close() throws IOException {}
    };
  }

  @Override
  public QueryTask issueQueries(final List<Query> queries)
  throws IOException {
    return new MultiQueryTask(queries) {

      @Override
      public Measurement call() {
        final Measurement measurement = new Measurement(100);
        measurement.setCPUTime(1000000000);
        return measurement;
      }

      @Override
      public void close() throws IOException {}
    };
  }

  @Override
  public TermFreqIterator getTermFreqIterator() {
    return new MockTermFreqIterator();
  }

}
