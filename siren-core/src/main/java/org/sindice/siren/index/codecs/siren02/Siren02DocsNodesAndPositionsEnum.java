 ...
 * term's payload.
 */
public class Siren02DocsNodesAndPositionsEnum extends DocsNodesAndPositionsEnum {
  ...
  public Siren02DocsNodesAndPositionsEnum(final DocsAndPositionsEnum e) {
    this.e = e;
  }
  ...
  public boolean nextNode() throws IOException {
    ...
    do {
      if (++_posPtr < e.freq()) {
        e.nextPosition();
        this.decodePayload();
        nextPos = codec.getPosition(); // backup position in nextPos
  ...
  private IntsRef decodeNodePath(IntsRef dst) throws IOException {
    final IntsRef nodePath = codec.getNode();
    ...
    dst.ints[0] = curNode.ints[curNode.offset] == -1 ? nodePath.ints[nodePath.offset] : curNode.ints[curNode.offset] + nodePath.ints[nodePath.offset];

    for (int i = 1, j = nodePath.offset + 1, k = curNode.offset + 1; i < dst.length; i++, j++, k++) {
      dst.ints[i] = (i >= curNode.length || curNode.ints[k] == -1 || nodePath.ints[j-1] != 0) ? nodePath.ints[j] : curNode.ints[k] + nodePath.ints[j];
    }
