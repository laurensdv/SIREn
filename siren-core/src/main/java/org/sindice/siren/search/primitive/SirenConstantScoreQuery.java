 ...
 */
public class SirenConstantScoreQuery extends NodePrimitiveQuery {

  protected final NodePrimitiveQuery query;

  /** Strips off scores from the passed in Query. The hits will get a constant score
   * dependent on the boost factor of this query. */
  public SirenConstantScoreQuery(final NodePrimitiveQuery query) {
    if (query == null)
...
//  }
  ...
  @Override
  public NodePrimitiveQuery rewrite(final IndexReader reader) throws IOException {
    if (query != null) {
      NodePrimitiveQuery rewritten = (NodePrimitiveQuery) query.rewrite(reader);
      if (rewritten != query) {
...
//      } else {
      ...
      it = (NodeScorer) innerWeight.scorer(reader, scoreDocsInOrder, topScorer);
...
//      }
  ...
  protected class SirenConstantScorer extends NodePrimitiveScorer {
        ...
        public void setScorer(final Scorer scorer) throws IOException {
          ...
          collector.setScorer(
            new SirenConstantScorer(
              ...
              (NodeScorer) scorer, SirenConstantScorer.this.weight));
        }
    ...
    public void score(final Collector collector) throws IOException {
      ((NodeScorer) it).score(this.wrapCollector(collector));
    }
    ...
    throws IOException {
      return ((NodeScorer) it).score(this.wrapCollector(collector), max, firstDocID);
    }
