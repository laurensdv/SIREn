...
import org.sindice.siren.search.node.NodeDisjunctionScorerQueue;
 ...
 * use case.
 */
class SirenCellDisjunctionScorer extends NodeScorer {
   ...
   * are exhausted.
   */
  private NodeDisjunctionScorerQueue                    scorerCellQueue = null;
...
//  private final int                          dataset     = -1;
  ...
  throws IOException {
    ...
    scorerCellQueue = new NodeDisjunctionScorerQueue(nrScorers);
    queueSize = 0;
    while (si.hasNext()) {
  ...
  public int nextDocument() throws IOException {
    if (scorerCellQueue == null) {
      ...
      if ((nrMatchers = scorerCellQueue.nrMatches()) > 0) {
        entity = scorerCellQueue.topDocument();
  ...
  public int nextPosition() throws IOException {
    ...
    if (scorerCellQueue.topNode()[0] == Integer.MAX_VALUE) {
      return NO_MORE_POS;
    }
    entity = scorerCellQueue.topDocument();
    tuple[0] = scorerCellQueue.topNode()[0];
    currentScore = 0;
    nrMatchers = 0;
    while (scorerCellQueue.topDocument() == entity &&
           scorerCellQueue.topNode()[0] == tuple[0]) { // while top is a match, advance
  ...
  public int skipTo(final int entityID) throws IOException {
    ...
    while (queueSize > 0) {
      if (scorerCellQueue.topDocument() >= entityID) {
        entity = scorerCellQueue.topDocument();
        return doc();
      }
  ...
  throws IOException {
    ...
    while (queueSize > 0) {
      if (scorerCellQueue.topDocument() > entityID ||
         (scorerCellQueue.topDocument() == entityID && scorerCellQueue.topNode()[0] >= node()[0])) {
        entity = scorerCellQueue.topDocument();
        tuple[0] = scorerCellQueue.topNode()[0];
        return doc();
      }
  ...
  @Override
  public int doc() {
    return scorerCellQueue.topDocument();
  }
